...
const handler = async (req: NextApiRequest, res: NextApiResponse<any>) => {
  try {
    const { messages, key, model, googleAPIKey} =
      req.body as GoogleBody;
    const myHeaders = new Headers();
    myHeaders.append("X-API-KEY", req.body.googleAPIKey);
    ...
    const googleRes = await fetch("https://google.serper.dev/search", requestOptions);

    console.log(googleRes);

    const googleData = await googleRes.json();
    const source = googleData.organic[0] 
    const sources: GoogleSource[] = googleData.organic.map((item: any) => ({
      title: item.title,
      link: item.link,
